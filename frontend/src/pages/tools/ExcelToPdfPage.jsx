// src/pages/tools/ExcelToPdfPage.jsx
...
const ExcelToPdfPage = () => {
  ...
  } = usePdfTool('excel_to_pdf'); // Operation name matches backend

  const handleFileSelected = useCallback((files) => {
    clearError(); // Clears toolError from the hook
  ...
  const handleConvert = async () => {
    clearError();
    if (!selectedFile) {
      // This is a client-side validation, ideally FileUploadZone or button disabling handles this.
      // For now, a simple alert. You might want a more integrated way to show this type of error.
  ...
  // Directly use toolError.message if toolError exists, otherwise it's an empty string
  const displayErrorMessage = toolError ? toolError.message : '';
  // Ensure successMessage is also an object with a 'message' key from backend, or handle it
  const displaySuccessMessage = successMessage ? successMessage.message : "Your Excel file has been converted to PDF.";


  // For debugging what toolError contains:
  useEffect(() => {
    if (toolError) {
      console.log("Tool Error from hook:", JSON.stringify(toolError));
    }
    if (successMessage) {
      console.log("Success Message from hook:", JSON.stringify(successMessage));
    }
  ...
  const friendlyAcceptedFileTypes = ".xls, .xlsx"; // For display
          ...
          >
            <div className="text-center mb-8 sm:mb-12">
              ...
              <p className={`mt-3 sm:mt-4 text-base sm:text-lg max-w-2xl mx-auto ${isDarkMode ? 'text-slate-400' : 'text-slate-600'}`}>
                Transform your .xls and .xlsx spreadsheets into easily shareable PDF files.
              </p>
            </div>
            ...
            {!processedFileUrl && !isLoading ? (
              <motion.div
                key="upload-excel-area" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
              ...
              >
                {!selectedFile ? (
                  <FileUploadZone
                      ...
                      maxFileSizeMB={50}
                  >
                      <FaFileExcel className={`w-10 h-10 sm:w-12 sm:h-12 mb-3 sm:mb-4 ${isDarkMode ? 'text-sky-400' : 'text-sky-600'}`} />
                      <p className={`text-base sm:text-lg font-semibold ${isDarkMode ? 'text-slate-200' : 'text-slate-700'}`}>Drag & Drop Excel File Here</p>
                  ...
                  >
                    <div className="flex justify-between items-center mb-4">
                        <div className="flex items-center overflow-hidden">
                            <FaFileExcel className={`w-8 h-8 mr-3 flex-shrink-0 ${isDarkMode ? 'text-sky-400' : 'text-sky-600'}`} />
                            <div>
                ...
                {/* Error display: only show if displayErrorMessage has content */}
                {displayErrorMessage && (
                ...
                {/* Convert button: only show if file is selected AND not loading AND no file already processed */}
            ...
            ) : isLoading ? ( <motion.div key="loading-excel" initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}} className="mt-8 text-center max-w-md mx-auto">
            ...
            ) : processedFileUrl ? ( <motion.div key="results-excel-area" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="max-w-md mx-auto text-center p-6 sm:p-8 rounded-xl shadow-xl border ${isDarkMode ? 'bg-slate-800 border-slate-700' : 'bg-white border-slate-200'}">
