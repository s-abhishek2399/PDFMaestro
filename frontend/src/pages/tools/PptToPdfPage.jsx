// src/pages/tools/PptToPdfPage.jsx
...
const PptToPdfPage = () => {
  ...
  const [clientSideError, setClientSideError] = useState(''); // For errors like "no file selected"
  ...
  } = usePdfTool('ppt_to_pdf'); // Operation name matches backend
  ...
  const handleConvert = async () => {
    ...
    setClientSideError('');
    if (!selectedFile) {
      setClientSideError("Please select a PowerPoint file to convert."); 
      return;
    }
    await processFiles([selectedFile]); // No extra options needed for simple conversion
  };
  ...
  // Combine client-side and tool errors for display
  const displayErrorMessage = clientSideError || (toolError ? toolError.message : '');
  // Ensure successMessage is the direct string from the hook
  const displaySuccessMessage = successMessage ? successMessage.message : "Your PowerPoint file has been converted to PDF.";

  // For debugging hook states
  useEffect(() => {
    if (toolError) console.log("PPTtoPDF Tool Error:", JSON.stringify(toolError));
    if (successMessage) console.log("PPTtoPDF Success Message:", JSON.stringify(successMessage));
  }, [toolError, successMessage]);

  const acceptedFileTypes = "application/vnd.ms-powerpoint,application/vnd.openxmlformats-officedocument.presentationml.presentation";
  const friendlyAcceptedFileTypes = ".ppt, .pptx";
          ...
          >
            <div className="text-center mb-8 sm:mb-12">
              ...
              <p className={`mt-3 sm:mt-4 text-base sm:text-lg max-w-2xl mx-auto ${isDarkMode ? 'text-slate-400' : 'text-slate-600'}`}>
                Easily convert your .ppt and .pptx presentations to high-quality PDF documents.
              </p>
            </div>
            ...
            {!processedFileUrl && !isLoading ? (
              <motion.div
                key="upload-ppt-area" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
              ...
              >
                {!selectedFile ? (
                  <FileUploadZone
                      ...
                      disabled={isLoading} 
                      maxFileSizeMB={100} // PPTs can be large
                  >
                      <FaFilePowerpoint className={`w-10 h-10 sm:w-12 sm:h-12 mb-3 sm:mb-4 ${isDarkMode ? 'text-sky-400' : 'text-sky-600'}`} />
                      <p className={`text-base sm:text-lg font-semibold ${isDarkMode ? 'text-slate-200' : 'text-slate-700'}`}>Drag & Drop PowerPoint File Here</p>
                      <p className={`${isDarkMode ? 'text-slate-400' : 'text-slate-500'} text-sm`}>or click to select a file</p>
                      <p className={`mt-2 text-xs ${isDarkMode ? 'text-slate-500' : 'text-slate-400'}`}>Supports: {friendlyAcceptedFileTypes}. Max 100MB.</p>
                  </FileUploadZone>
                ) : (
                  <motion.div 
                  ...
                  >
                    <div className="flex justify-between items-center mb-4">
                        <div className="flex items-center overflow-hidden">
                            <FaFilePowerpoint className={`w-8 h-8 mr-3 flex-shrink-0 ${isDarkMode ? 'text-sky-400' : 'text-sky-600'}`} />
                            <div>
                ...
                {displayErrorMessage && (
                   <div className="mt-6 text-center">
                        ...
                        >
                            <FiAlertTriangle className="w-5 h-5 mr-2"/> {displayErrorMessage}
                        </motion.div>
                   </div>
                )}
                ...
                {selectedFile && !isLoading && !processedFileUrl && (
                   <div className="mt-6 text-center">
                        <button 
                            ...
                            disabled={isLoading}
                            className={`px-8 py-3.5 text-base sm:text-lg font-semibold rounded-lg shadow-md transition-all duration-200 ease-in-out focus:outline-none focus:ring-4 transform hover:scale-105 active:scale-95 ${isDarkMode ? 'bg-sky-500 hover:bg-sky-400 text-white focus:ring-sky-500/50' : 'bg-sky-600 hover:bg-sky-700 text-white focus:ring-sky-600/50'} ${isLoading ? 'opacity-50 cursor-not-allowed' : ''}`}
                        >
                            <FiUploadCloud className="inline w-5 h-5 mr-2 -mt-0.5" /> Convert to PDF
                        </button>
                   </div>
                )}
              </motion.div>
            ) : isLoading ? ( 
              <motion.div key="loading-ppt" initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}} className="mt-8 text-center max-w-md mx-auto">
              ...
              <motion.div key="results-ppt-area" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="max-w-md mx-auto text-center p-6 sm:p-8 rounded-xl shadow-xl border ${isDarkMode ? 'bg-slate-800 border-slate-700' : 'bg-white border-slate-200'}">
                ...
                >
                  <FiDownload className="mr-2 w-5 h-5" /> Download PDF
                </a>
                <button 
                  onClick={resetAll} 
                ...
                >
                  <FiRotateCcw className="mr-2 w-4 h-4" /> Convert Another File
                </button>
              </motion.div>
            ) : null}
            </AnimatePresence>
