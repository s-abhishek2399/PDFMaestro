// src/pages/tools/PdfToWordPage.js
...
import { FiFileText, FiTrash2, FiUploadCloud, FiDownload, FiRotateCcw, FiAlertTriangle, FiFileMinus } from 'react-icons/fi'; // FiFileMinus as icon
...
const PdfToWordPage = () => {
  ...
  const removeFile = useCallback(() => {
    setSelectedFile(null); setLocalError(''); clearError();
  }, [clearError]);

  const handleConvertToWord = async () => {
    clearError(); setLocalError('');
    if (!selectedFile) {
      setLocalError("Please select a PDF file to convert."); return;
    }
    await processFiles([selectedFile], {}); // No specific options for this basic version
  };
  ...
  const MAX_PDF_SIZE_MB = 25; // Conversion can be resource-intensive
          ...
          >
            <div className="text-center mb-8 sm:mb-12">
              ...
              <p className={`mt-3 sm:mt-4 text-base sm:text-lg max-w-2xl mx-auto ${isDarkMode ? 'text-slate-400' : 'text-slate-600'}`}>
                Transform your PDF documents into editable Word files (.docx) with ease.
              </p>
            </div>
            ...
            {!processedFileUrl && !isLoading ? (
              <motion.div
                key="upload-pdf-to-word-area" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
                className="max-w-xl mx-auto"
              >
                {!selectedFile ? (
                  <FileUploadZone
                      onFilesSelected={handleFileSelected} acceptedTypes="application/pdf" multiple={false}
                      isDarkMode={isDarkMode} disabled={isLoading} maxFileSizeMB={MAX_PDF_SIZE_MB}
                  >
                      ...
                      <p className={`mt-2 text-xs ${isDarkMode ? 'text-slate-500' : 'text-slate-400'}`}>Supports: .pdf files only. Max {MAX_PDF_SIZE_MB}MB.</p>
                  </FileUploadZone>
                ) : (
                  <motion.div 
                ...
                {(selectedFile || currentError) && (
                   <div className="mt-6 text-center">
                      ...
                      {selectedFile && (
                          <button onClick={handleConvertToWord} className={`px-8 py-3.5 text-base sm:text-lg font-semibold rounded-lg shadow-md transition-all duration-200 ease-in-out focus:outline-none focus:ring-4 transform hover:scale-105 active:scale-95 ${isDarkMode ? 'bg-sky-500 hover:bg-sky-400 text-white focus:ring-sky-500/50' : 'bg-sky-600 hover:bg-sky-700 text-white focus:ring-sky-600/50'}`}
                          ><FiFileMinus className="inline w-5 h-5 mr-2 -mt-0.5" /> Convert to Word</button>
                      )}
                   </div>
                )}
              </motion.div>
            ) : isLoading ? ( <motion.div key="loading-pdf-to-word" initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}} className="mt-8 text-center max-w-md mx-auto">
                <div className={`text-lg font-semibold mb-3 ${isDarkMode ? 'text-sky-400' : 'text-sky-600'}`}>Converting to Word...</div>
                <p className={`text-sm mb-3 ${isDarkMode ? 'text-slate-400' : 'text-slate-500'}`}>This may take a few moments for complex PDFs.</p>
            ...
            ) : processedFileUrl ? ( <motion.div key="results-pdf-to-word-area" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="max-w-md mx-auto text-center p-6 sm:p-8 rounded-xl shadow-xl border ${isDarkMode ? 'bg-slate-800 border-slate-700' : 'bg-white border-slate-200'}">
                ...
                <p className={`mb-4 text-sm ${isDarkMode ? 'text-slate-300' : 'text-slate-600'}`}>{successMessage?.message || "Your PDF has been converted to a Word document."}</p>
                ...
                ><FiDownload className="mr-2 w-5 h-5" /> Download DOCX</a>
                ...
                ><FiRotateCcw className="mr-2 w-4 h-4" /> Convert Another PDF</button>
              </motion.div>
            ) : null}
            </AnimatePresence>
          </motion.div>
