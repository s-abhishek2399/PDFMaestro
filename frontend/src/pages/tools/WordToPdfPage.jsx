// src/pages/tools/WordToPdfPage.js
...
import { 
    FaFileWord, // From Font Awesome
    // Keep other Fa icons here if any
} from 'react-icons/fa'; 
import { 
    FiUploadCloud, FiDownload, FiAlertTriangle, // From Feather Icons
    FiRotateCcw, FiCheckCircle, FiTrash2 // Also, you likely want FiTrash2 for the remove button
} from 'react-icons/fi';
...
const WordToPdfPage = () => {
  ...
  } = usePdfTool('word_to_pdf'); // Operation name matches backend

  useEffect(() => {
    if (toolError) {
      setLocalError(toolError.message || (toolError.error ? toolError.error.message : 'An unknown error occurred.'));
    } else {
      setLocalError('');
    }
  }, [toolError]);
  ...
  const handleFileSelected = useCallback((files) => {
    ...
    if (files && files.length > 0) {
      const file = files[0];
      // FileUploadZone should handle type validation based on acceptedTypes prop
  ...
  const removeFile = useCallback(() => {
    setSelectedFile(null); resetPdfToolHook(); clearError(); setLocalError('');
  }, [clearError, resetPdfToolHook]);

  const handleConvert = async () => {
    clearError(); setLocalError('');
    if (!selectedFile) {
      setLocalError("Please select a Word file to convert."); return;
    }
  ...
  const acceptedFileTypes = "application/msword, application/vnd.openxmlformats-officedocument.wordprocessingml.document";
          ...
          >
            <div className="text-center mb-8 sm:mb-12">
              ...
              <p className={`mt-3 sm:mt-4 text-base sm:text-lg max-w-2xl mx-auto ${isDarkMode ? 'text-slate-400' : 'text-slate-600'}`}>
                Easily convert your .doc and .docx files to high-quality PDF documents.
              </p>
            </div>
            ...
            {!processedFileUrl && !isLoading ? (
              <motion.div
                key="upload-convert-area" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
              ...
              >
                {!selectedFile ? (
                  <FileUploadZone
                      ...
                      disabled={isLoading} 
                      maxFileSizeMB={50} // Example max size
                  >
                      <FaFileWord className={`w-10 h-10 sm:w-12 sm:h-12 mb-3 sm:mb-4 ${isDarkMode ? 'text-sky-400' : 'text-sky-600'}`} />
                      <p className={`text-base sm:text-lg font-semibold ${isDarkMode ? 'text-slate-200' : 'text-slate-700'}`}>Drag & Drop Word File Here</p>
                      <p className={`${isDarkMode ? 'text-slate-400' : 'text-slate-500'} text-sm`}>or click to select a file</p>
                      <p className={`mt-2 text-xs ${isDarkMode ? 'text-slate-500' : 'text-slate-400'}`}>Supports: .doc, .docx. Max 50MB.</p>
                  </FileUploadZone>
                ) : (
                  <motion.div 
                  ...
                  >
                    <div className="flex justify-between items-center mb-4">
                        <div className="flex items-center overflow-hidden">
                            <FaFileWord className={`w-8 h-8 mr-3 flex-shrink-0 ${isDarkMode ? 'text-sky-400' : 'text-sky-600'}`} />
                            <div>
                        ...
                        ><FiAlertTriangle className="w-5 h-5" /> {/* Or FiTrash2 */} </button>
                    </div>
                    {/* No specific options for Word to PDF conversion generally */}
                  </motion.div>
                )}
                ...
                {(selectedFile || currentError) && (
                   <div className="mt-6 text-center">
                      ...
                      {selectedFile && !currentError && ( // Only show button if file selected and no pre-existing local error
                          <button onClick={handleConvert} className={`px-8 py-3.5 text-base sm:text-lg font-semibold rounded-lg shadow-md transition-all duration-200 ease-in-out focus:outline-none focus:ring-4 transform hover:scale-105 active:scale-95 ${isDarkMode ? 'bg-sky-500 hover:bg-sky-400 text-white focus:ring-sky-500/50' : 'bg-sky-600 hover:bg-sky-700 text-white focus:ring-sky-600/50'}`}
                          ><FiUploadCloud className="inline w-5 h-5 mr-2 -mt-0.5" /> Convert to PDF</button>
                      )}
                   </div>
                )}
              </motion.div>
            ) : isLoading ? ( <motion.div key="loading-convert" initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}} className="mt-8 text-center max-w-md mx-auto">
            ...
            ) : processedFileUrl ? ( <motion.div key="results-convert-area" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="max-w-md mx-auto text-center p-6 sm:p-8 rounded-xl shadow-xl border ${isDarkMode ? 'bg-slate-800 border-slate-700' : 'bg-white border-slate-200'}">
                ...
                <p className={`mb-4 text-sm ${isDarkMode ? 'text-slate-300' : 'text-slate-600'}`}>{successMessage?.message || "Your Word file has been converted to PDF."}</p>
                ...
                ><FiRotateCcw className="mr-2 w-4 h-4" /> Convert Another File</button>
            </motion.div>
            ) : null}
            </AnimatePresence>
          </motion.div>
