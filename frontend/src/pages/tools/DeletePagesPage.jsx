// src/pages/tools/DeletePagesPage.jsx
...
const DeletePagesPage = () => {
  ...
  } = usePdfTool('delete_pages');

  const validatePageRanges = (ranges, maxPages) => {
    if (!ranges.trim()) { setPageRangeError('Please enter page numbers to delete.'); return false; }
    ...
    const selectedIndicesForDeletion = new Set();
    ...
    for (const part of parts) {
      ...
      if (trimmedPart.includes('-')) {
        ...
        for(let i = start; i <= end; i++) selectedIndicesForDeletion.add(i-1);
        ...
        selectedIndicesForDeletion.add(page-1);
        hasValidSelection = true;
      }
    }
    if (!hasValidSelection && ranges.trim() !== "") {
        setPageRangeError('No valid page numbers entered.'); return false;
    }
    if (hasValidSelection && maxPages > 0 && selectedIndicesForDeletion.size === maxPages) {
        setPageRangeError("Cannot delete all pages from the PDF. Please leave at least one page.");
        return false;
    }
  ...
  const handleFileSelected = useCallback((files) => {
    clearError(); setClientSideError(''); setTotalPages(0); setPagesToDelete(''); setPageRangeError(''); setNewTotalPages(0);
  ...
  const removeFile = useCallback(() => {
    ...
    setTotalPages(0); setPagesToDelete(''); setPageRangeError(''); setNewTotalPages(0);
  ...
  const handleAction = async () => {
    ...
    if (!pagesToDelete.trim()) { setPageRangeError("Please specify pages to delete."); return; }
    if (totalPages > 0 && !validatePageRanges(pagesToDelete, totalPages)) return;

    await processFiles([selectedFile], { pagesToDelete });
  };
  
  const resetAll = () => {
    setSelectedFile(null); resetPdfToolHook(); clearError(); setClientSideError('');
    setPagesToDelete(''); setTotalPages(0); setPageRangeError(''); setNewTotalPages(0);
  };
  ...
  useEffect(() => {
    ...
    if (messageSource) {
        ...
        if (typeof messageSource.newTotalPages !== 'undefined') newPagesCount = parseInt(messageSource.newTotalPages, 10);
    }
    
    if (!isNaN(originalPages) && originalPages >= 0 && totalPages !== originalPages) setTotalPages(originalPages);
    if (!isNaN(newPagesCount) && newPagesCount >= 0 && newTotalPages !== newPagesCount) setNewTotalPages(newPagesCount);

  }, [successMessage, toolError, totalPages, newTotalPages]);

  const currentCombinedError = clientSideError || pageRangeError || (toolError ? toolError.message : '');
  const displaySuccessMessage = successMessage ? successMessage.message : (processedFileUrl ? "Pages have been deleted successfully." : "");
          ...
          <motion.div initial={{ opacity: 0, y:20 }} animate={{ opacity: 1, y:0 }} transition={{ duration: 0.5 }}>
            <div className="text-center mb-8 sm:mb-12">
              ...
              <p className={`mt-3 sm:mt-4 text-base sm:text-lg max-w-2xl mx-auto ${isDarkMode ? 'text-slate-400' : 'text-slate-600'}`}>
                Remove specific unwanted pages from your PDF document.
              </p>
            </div>
            ...
            <AnimatePresence mode="wait">
              {!processedFileUrl && !isLoading ? (
                ...
                >
                  {!selectedFile ? (
                    ...
                    <motion.div initial={{opacity:0, y:10}} animate={{opacity:1,y:0}}
                      ...
                      <div className="mt-4">
                          <label htmlFor="pagesToDelete" className={`block text-sm font-medium mb-1 ${isDarkMode ? 'text-slate-300' : 'text-slate-700'}`}>
                              Pages to Delete: {totalPages > 0 && <span className="text-xs text-slate-400">(Total: {totalPages} pages)</span>}
                          </label>
                          <input type="text" id="pagesToDelete" value={pagesToDelete}
                              onChange={(e) => { setPagesToDelete(e.target.value); if (totalPages > 0) validatePageRanges(e.target.value, totalPages); else setPageRangeError(''); }}
                              placeholder="e.g., 1, 3-5, 7 (cannot be all pages)"
                  ...
                  {currentCombinedError && ( 
                      <div className="mt-6 text-center"> 
                          ...
                          >
                              <FiAlertTriangle className="w-5 h-5 mr-2"/> {currentCombinedError}
                          </motion.div>
                      </div>
                  )}

                  {selectedFile && !isLoading && !processedFileUrl && ( 
                      <div className="mt-6 text-center">
                          <button onClick={handleAction} disabled={isLoading || !!pageRangeError} 
                              className={`px-8 py-3.5 text-base sm:text-lg font-semibold rounded-lg shadow-md transition-all duration-200 ease-in-out focus:outline-none focus:ring-4 transform hover:scale-105 active:scale-95 ${isDarkMode ? 'bg-red-500 hover:bg-red-400 text-white focus:ring-red-500/50' : 'bg-red-600 hover:bg-red-700 text-white focus:ring-red-600/50'} ${(isLoading || !!pageRangeError) ? 'opacity-50 cursor-not-allowed' : ''}`}
                          >
                              <FaTrashAlt className="inline w-4 h-4 mr-2 -mt-0.5" /> Delete Pages
                          </button>
                      </div>
                  )}
                </motion.div> 
              ) : isLoading ? ( 
                <motion.div key="loading-deletepages" initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}} className="mt-8 text-center max-w-md mx-auto">
                  <div className={`text-lg font-semibold mb-3 ${isDarkMode ? 'text-sky-400' : 'text-sky-600'}`}>Deleting Pages...</div>
                  <div className={`w-full ${isDarkMode ? 'bg-slate-700' : 'bg-slate-200'} rounded-full h-2.5`}>
                      <motion.div className={`h-2.5 rounded-full ${isDarkMode ? 'bg-sky-500' : 'bg-sky-600'}`} initial={{ width: '0%' }} animate={{ width: `${progress}%` }} transition={{ duration: 0.3, ease:"linear" }}/>
                  </div>
                ...
                <motion.div 
                    key="results-deletepages-area" 
                    initial={{ opacity: 0, y: 20 }} 
                    animate={{ opacity: 1, y: 0 }} 
                    // CORRECTED className HERE:
                    className={`max-w-md mx-auto text-center p-6 sm:p-8 rounded-xl shadow-xl border ${isDarkMode ? 'bg-slate-800 border-slate-700' : 'bg-white border-slate-200'}`}
                >
                  <FiCheckCircle className={`w-12 h-12 mx-auto mb-4 ${isDarkMode ? 'text-green-400' : 'text-green-600'}`} />
                  <h2 className={`text-2xl font-semibold mb-2 ${isDarkMode ? 'text-slate-100' : 'text-slate-800'}`}>Pages Deleted!</h2>
