// src/pages/tools/PdfToExcelPage.jsx
...
import { FaFilePdf, FaFileExcel } from 'react-icons/fa'; // Using FaFilePdf for input, FaFileExcel for concept
import { 
...
const PdfToExcelPage = () => {
  ...
  const handleConvert = async () => {
    clearError(); setClientSideError('');
    if (!selectedFile) {
      setClientSideError("Please select a PDF file to convert to Excel."); return;
    }
  ...
  const displaySuccessMessage = successMessage ? successMessage.message : "Tables from your PDF have been extracted to Excel.";
  
  useEffect(() => {
    if (toolError) console.log("PdfToExcel Tool Error:", JSON.stringify(toolError));
    if (successMessage) console.log("PdfToExcel Success Message:", JSON.stringify(successMessage));
  }, [toolError, successMessage]);

  const acceptedFileTypes = "application/pdf";
  const friendlyAcceptedFileTypes = ".pdf";
          ...
          <motion.div initial={{ opacity: 0, y:20 }} animate={{ opacity: 1, y:0 }} transition={{ duration: 0.5 }}>
            <div className="text-center mb-8 sm:mb-12">
              ...
              <p className={`mt-3 sm:mt-4 text-base sm:text-lg max-w-2xl mx-auto ${isDarkMode ? 'text-slate-400' : 'text-slate-600'}`}>
                Extract tables from your PDF documents and save them as Excel (.xlsx) files.
              </p>
            </div>
            <AnimatePresence mode="wait">
            {!processedFileUrl && !isLoading ? (
              <motion.div key="upload-pdf2excel-area" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="max-w-xl mx-auto">
                {!selectedFile ? (
                  <FileUploadZone
                      ...
                      <p className={`mt-2 text-xs ${isDarkMode ? 'text-slate-500' : 'text-slate-400'}`}>Supports: {friendlyAcceptedFileTypes}. Max 50MB.</p>
                  </FileUploadZone>
                ) : (
