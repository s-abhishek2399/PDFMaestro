// src/pages/tools/PdfToPptPage.jsx
...
const PdfToPptPage = () => {
  ...
  const handleConvert = async () => {
    clearError(); setClientSideError('');
    if (!selectedFile) {
      setClientSideError("Please select a PDF file to convert to PowerPoint."); return;
    }
  ...
  const displaySuccessMessage = successMessage ? successMessage.message : "Your PDF has been converted to PowerPoint.";
  
  useEffect(() => {
    if (toolError) console.log("PdfToPpt Tool Error:", JSON.stringify(toolError));
    if (successMessage) console.log("PdfToPpt Success Message:", JSON.stringify(successMessage));
          ...
          <motion.div initial={{ opacity: 0, y:20 }} animate={{ opacity: 1, y:0 }} transition={{ duration: 0.5 }}>
            <div className="text-center mb-8 sm:mb-12">
              ...
              <p className={`mt-3 sm:mt-4 text-base sm:text-lg max-w-2xl mx-auto ${isDarkMode ? 'text-slate-400' : 'text-slate-600'}`}>
                Convert your PDF documents into editable PowerPoint (.pptx) presentations.
              </p>
            </div>
            <AnimatePresence mode="wait">
            {!processedFileUrl && !isLoading ? (
              <motion.div key="upload-pdf2ppt-area" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="max-w-xl mx-auto">
                {!selectedFile ? (
                  <FileUploadZone
                      onFilesSelected={handleFileSelected} acceptedTypes={acceptedFileTypes}
                      multiple={false} isDarkMode={isDarkMode} disabled={isLoading} maxFileSizeMB={100}> {/* PDFs for PPT can be large */}
