// src/pages/tools/TextToPdfPage.jsx
...
import { FiFileText } from 'react-icons/fi'; // Text icon
import { 
...
const TextToPdfPage = () => {
  ...
  const handleConvert = async () => {
    clearError(); setClientSideError('');
    if (!selectedFile) {
      setClientSideError("Please select a text file to convert."); return;
    }
  ...
  const displaySuccessMessage = successMessage ? successMessage.message : "Your text file has been converted to PDF.";
  
  useEffect(() => {
    if (toolError) console.log("TextToPDF Tool Error:", JSON.stringify(toolError));
    if (successMessage) console.log("TextToPDF Success Message:", JSON.stringify(successMessage));
  }, [toolError, successMessage]);

  const acceptedFileTypes = "text/plain,.md,.rtf"; // .txt, .md, .rtf
  const friendlyAcceptedFileTypes = ".txt, .md, .rtf";
          ...
          <motion.div initial={{ opacity: 0, y:20 }} animate={{ opacity: 1, y:0 }} transition={{ duration: 0.5 }}>
            <div className="text-center mb-8 sm:mb-12">
              ...
              <p className={`mt-3 sm:mt-4 text-base sm:text-lg max-w-2xl mx-auto ${isDarkMode ? 'text-slate-400' : 'text-slate-600'}`}>
                Convert your plain text (.txt, .md, .rtf) files into PDF documents.
              </p>
            </div>
            <AnimatePresence mode="wait">
            {!processedFileUrl && !isLoading ? (
              <motion.div key="upload-text-area" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="max-w-xl mx-auto">
                {!selectedFile ? (
                  <FileUploadZone
                      onFilesSelected={handleFileSelected} acceptedTypes={acceptedFileTypes}
                      multiple={false} isDarkMode={isDarkMode} disabled={isLoading} maxFileSizeMB={10}>
                      <FiFileText className={`w-10 h-10 sm:w-12 sm:h-12 mb-3 sm:mb-4 ${isDarkMode ? 'text-sky-400' : 'text-sky-600'}`} />
                      <p className={`text-base sm:text-lg font-semibold ${isDarkMode ? 'text-slate-200' : 'text-slate-700'}`}>Drag & Drop Text File Here</p>
                      <p className={`${isDarkMode ? 'text-slate-400' : 'text-slate-500'} text-sm`}>or click to select a file</p>
                      <p className={`mt-2 text-xs ${isDarkMode ? 'text-slate-500' : 'text-slate-400'}`}>Supports: {friendlyAcceptedFileTypes}. Max 10MB.</p>
                  </FileUploadZone>
                ) : (
                    ...
                    <div className="flex justify-between items-center mb-4">
                        <div className="flex items-center overflow-hidden">
                            ...
                            <div>
                                ...
                                <p className={`text-xs ${isDarkMode ? 'text-slate-400' : 'text-slate-500'}`}>Size: {formatFileSize(selectedFile.size)}</p>
                            </div>
                        </div>
                        <button onClick={removeFile} aria-label="Remove file" className={`p-1.5 rounded-full transition-colors focus:outline-none focus:ring-2 ${isDarkMode ? 'text-slate-400 hover:text-red-400 hover:bg-slate-600 focus:ring-red-500' : 'text-slate-500 hover:text-red-500 hover:bg-slate-300 focus:ring-red-600'}`}>
